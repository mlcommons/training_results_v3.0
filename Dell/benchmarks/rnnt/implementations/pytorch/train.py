...
def parse_args():
    ...
    training.add_argument('--steps_this_job', default=None, type=int,
                          help='Run for a number of steps with no effect on the lr schedule.')
...
def apply_ema(optimizer, model, ema_model, decay, ema_update_type):
    ...
    if ema_update_type == "fp32":
        raise NotImplementedError('ema_update_type fp32 no longer supported')
...
def main():
    ...
    assert args.dist_lamb, 'Only DistributedFusedLAMB optimizer is currently supported'
    ...
    for epoch in range(start_epoch + 1, args.epochs + 1):
            ...
            if accumulated_batches % args.grad_accumulation_steps == 0:
                ...
                if step % args.log_frequency == 0:
                    ...
                    dict_log = {'loss': step_loss,
                                ...
                                'took': step_time,
                                'lrate': optimizer._lr.item()} # TODO: eliminate sync
                ...
                step += 1
                accumulated_batches = 0
                if args.steps_this_job is not None and step > args.steps_this_job:
                    break
                # end of step
            sbridge.stop_prof(SBridge.ITER_TIME)
        ...
        if args.steps_this_job is not None and step > args.steps_this_job:
            print_once(f'Finished after {step - 1} steps (steps_this_job reached).')
            break
        if last_wer <= args.target:
            mllogger.log_run_stop(status=mllogger.constants.SUCCESS)
            print_once(f'Finished after {epoch} epochs (target reached).')
            break
        if 0 < args.epochs_this_job <= epoch - start_epoch:
            print_once(f'Finished after {args.epochs_this_job} epochs (epochs_this_job reached).')
            break
