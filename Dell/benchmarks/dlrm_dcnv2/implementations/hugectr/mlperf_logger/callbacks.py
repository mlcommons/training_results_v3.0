...
class LoggingCallback(hugectr.TrainingCallback):
    ...
    def on_training_end(self, current_iter: int):
        ...
        if not self._success:
            self.mllogger.log_run_stop(status=mlperf_constants.ABORTED, epoch_num=1)
        ...
        if self.mllogger.comm_handler.global_rank() == 0:
            if self._success:
                print(
                    ...
                    f"{current_iter} / {self.max_iter} iterations with batchsize {self.batch_size} "
                    f"in {self._total_time:.2f}s. Average speed is {self._throughput:.2f} records/s."
                )
            else:
                print(
                    f"Finish {self.max_iter} iterations with "
                    f"batchsize: {self.batch_size} in {self._total_time:.2f}s."
                )
        self.mllogger.event(
            key="tracked_stats",
            metadata={"step": current_iter / self.max_iter},
    ...
    def on_eval_start(self, current_iter: int) -> bool:
        self.mllogger.start(
            ...
            metadata={mlperf_constants.EPOCH_NUM: current_iter / self.max_iter},
        )
        return False

    def on_eval_end(self, current_iter: int, eval_results: Dict[str, float]) -> bool:
        epoch_num = current_iter / self.max_iter
