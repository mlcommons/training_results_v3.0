...
if [ ${NVTX_FLAG} -gt 0 ]; then
  ...
  NSYS_OUT="/results/single_stage_detector_pytorch_${DGXNNODES}x${DGXNGPU}x${BATCHSIZE}_${DATESTAMP}_${SLURM_PROCID}_${SYNTH_DATA}_${DISABLE_CG}.nsys-rep"
  NSYS_CMD=('/nsight/bin/nsys' 'profile' '--capture-range' 'cudaProfilerApi' '--capture-range-end' 'stop' '--sample=none' '--cpuctxsw=none' '--trace=cuda,nvtx' '--stats' 'true' '--cuda-graph-trace=node' '-f' 'true' '-o' ${NSYS_OUT})
else
  NSYS_CMD=()
fi
...
if [ -n "${SLURM_LOCALID-}" ]; then
  ...
  if [ "${SLURM_NTASKS}" -gt "${SLURM_JOB_NUM_NODES}" ]; then
    ...
    CMD=( 'bindpcie' ${IB_BIND} '--' ${NSYS_CMD[@]} 'python' '-u' )
  else
    CMD=( ${NSYS_CMD[@]} 'python' '-u' )
  fi
else
