...
def init_cache(model_ptr, dataset, device, args, cache_sz):
    ...
    cache_batch_count = int(cache_sz / args.batch_size)
    assert(cache_batch_count > 0)
    for j in range(cache_batch_count):
        images, targets = [], []
        for i in range(args.batch_size):
            dataset_idx = j * cache_batch_count + i
            images.append(dataset[dataset_idx][0])
            targets.append(dataset[dataset_idx][1])
...
def get_cached_dataset(model, dataset, device, args, cache_sz=32, virtual_cache_sz_factor=32768):
