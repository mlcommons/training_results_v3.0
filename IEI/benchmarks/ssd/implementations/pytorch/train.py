...
def parse_args(add_help=True):
    ...
    parser.add_argument('--dali-cmn', default=0, type=int,
                        help='which version of DALI crop_mirror_normalize to use (0: experimental, 1: regular, 2: transpose-normalize')
    parser.add_argument('--dali-preallocate-height', default=0, type=int,
                        help='hint of largest image height for memory preallocation provided to DALI image decoder')
    parser.add_argument('--dali-preallocate-width', default=0, type=int,
    ...
    parser.add_argument('--enable-sharp', default=False, action="store_true", help='enable SHARP')
...
def main(args):
    ...
    if args.dali and (args.rank in args.train_ranks):
        ...
        data_loader = DaliDataIterator(data_path=args.train_data_path,
                                       ...
                                       cache=False,
                                       cmn=args.dali_cmn,
                                       preallocate_h=args.dali_preallocate_height,
                                       preallocate_w=args.dali_preallocate_width,
                                       seed=args.seed)
    ...
    if args.cuda_graphs and not args.cuda_graphs_syn:
        # TODO(ahmadki): clean graph capture. this will not work right now because whole_model_capture expects
        # lr_scheduler
        graphed_model, static_input, static_loss, static_prologues_out = \
            model_capture.whole_model_capture(model, optimizer, scaler, data_loader, args)

    # TODO(ahmadki): cleanup graph capture
...
# TODO(ahmadki): clean graph capture:
# 1. --cuda-graphs should enable all cuda graphs optimizations. disabling some graph optimizations (like capturing
#    the lr scheduler) should be done through a negative flag --cuda-graphs-no-lr-scheduler
# 2. graphable lr_scheduler should move inside the capture function
# 3. we should have two train methods, one for capture graph and one for normal training (selected with --cuda-graphs)
# 4. fix and unify --cuda-graphs-sync and no --cuda-graphs-sync
