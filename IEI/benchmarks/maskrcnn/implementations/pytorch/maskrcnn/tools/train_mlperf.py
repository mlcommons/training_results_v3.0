        ...
        for (is_training, images_per_gpu) in per_gpu_batch_sizes:
            ...
            for i, shape in enumerate(shapes):
                ...
                if batch_size_one and is_training:
                    dummy_target_bboxes = torch.empty([1,annotations_per_image,4], dtype=torch.float32, device=device).fill_(0)
                    dummy_target_objectness = torch.empty([1,annotations_per_image], dtype=torch.float32, device=device).fill_(0)
                    dummy_target_labels = torch.empty([1,annotations_per_image], dtype=torch.float32, device=device).fill_(1)
                    print("%d :: dummy_target_bboxes, dummy_target_objectness, dummy_target_labels = %s, %s, %s" % (get_rank(), str(list(dummy_target_bboxes.shape)), str(list(dummy_target_objectness.shape)), str(list(dummy_target_labels.shape))))
                    sample_args = (dummy_batch.clone(),dummy_image_sizes.clone(),dummy_target_bboxes,dummy_target_objectness,dummy_target_labels,)
                else:
