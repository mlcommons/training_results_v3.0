...
class MaskRCNNFPNFeatureExtractor(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.nhwc = cfg.NHWC
        self.fusion = cfg.MODEL.ROI_MASK_HEAD.FEATURE_EXTRACTOR_FUSION
        if self.fusion:
            assert(self.nhwc), "Error! mask head fusion requires nhwc"
        ...
        for layer_idx, layer_features in enumerate(layers, 1):
            ...
            if self.fusion:
                module = make_conv3x3(next_feature, layer_features, 
                    dilation=dilation, stride=1, use_gn=use_gn, nhwc=False
                )
                if self.nhwc:
                    module = module.to(memory_format=torch.channels_last)
            else:
                module = make_conv3x3(next_feature, layer_features, 
                    dilation=dilation, stride=1, use_gn=use_gn, nhwc=self.nhwc
                )
...
##            x = nchw_to_nhwc_transform(x)
        ...
        if self.fusion:
            ...
            for layer_name in self.blocks:
                conv = getattr(self, layer_name)
                x = ConvBiasReLU(x, conv.weight, conv.bias, 1, 1)
            x = x.permute([0,2,3,1])
        else:
            for layer_name in self.blocks:
