...
struct SGDFunctor
{
   __device__ __forceinline__ void operator()(
    ...
    const int* group_index_ptr,
    const float* optimizer_state_ptr,
    bool wd_after_momentum)
  {
    ...
    bool first_run = (optimizer_state_ptr[0] <= 0.0f) ? true : false;
    float scale = 1.0f / optimizer_state_ptr[2];

    int group_index = group_index_ptr[tensor_loc];
    const float* group_state = optimizer_state_ptr + 3 + group_index * 5;
    float wd = group_state[0];
    float momentum = group_state[1];
    float dampening = group_state[2];
    bool nesterov = (group_state[3] != 0.0f) ? true : false;
    float lr = group_state[4];
  ...
  bool wd_after_momentum)
{
  ...
  auto group_index_ptr = group_index.data_ptr<int>();
  auto optimizer_state_ptr = optimizer_state.data_ptr<float>();
