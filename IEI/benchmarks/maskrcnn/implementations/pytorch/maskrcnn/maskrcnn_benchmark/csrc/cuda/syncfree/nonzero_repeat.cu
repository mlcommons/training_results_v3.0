 ...
 */
#define CUB_WRAPPED_NAMESPACE mrcnn
#include <cub/cub.cuh>
...
void nonzero(cudaStream_t stream, T_f* d_values, T_i* d_shuffle, T_i num_items, T_i num_outputs, T_i* d_indices, T_i* d_counts, T_i counts_idx, bool cat_outputs, T_i* d_offsets, void* d_temp_storage, size_t temp_storage_bytes)
{
    ...
    CUB_NS_QUALIFIER::DeviceScan::ExclusiveSum(d_temp_storage, temp_storage_bytes, d_offsets, d_offsets, num_items, stream);
    check_cuda_error(__LINE__);
	...
	)
{
    ...
    AT_DISPATCH_INDEX_TYPES(indices.scalar_type(), "pyt_nonzero_repeat", [&]() {
	    ...
	    index_t* offsets_ptr = static_cast<index_t*>(offsets.get());
            ...
            cudaError_t err = CUB_NS_QUALIFIER::DeviceScan::ExclusiveSum(nullptr, temp_storage_bytes, indices.data_ptr<index_t>(), offsets_ptr, n);
            if (err != cudaSuccess) {
        ...
        )
{
    ...
    AT_DISPATCH_INDEX_TYPES(indices.scalar_type(), "pyt_balanced_pos_neg_sampler", [&]() {
        ...
        cudaError_t err = CUB_NS_QUALIFIER::DeviceScan::ExclusiveSum(nullptr, temp_storage_bytes_np, offsets_ptr, offsets_ptr, np);
        if (err != cudaSuccess) printf("mrcnn::cub::DeviceScan::ExclusiveSum returned %s\n", cudaGetErrorName(err));
        err = CUB_NS_QUALIFIER::DeviceScan::ExclusiveSum(nullptr, temp_storage_bytes_nn, offsets_ptr, offsets_ptr, nn);
        if (err != cudaSuccess) printf("mrcnn::cub::DeviceScan::ExclusiveSum returned %s\n", cudaGetErrorName(err));
