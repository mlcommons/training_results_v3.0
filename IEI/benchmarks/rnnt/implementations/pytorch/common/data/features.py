...
class VectorizedSpecAugment(SpecAugment):
    def calculate_features(self, x, x_lens):
        ...
        time_shape   = (torch.rand([b, self.time_masks, 1], device='cuda') * (xlen * self.max_time + 1)).int()
        time_anchors = (torch.rand([b, self.time_masks, 1], device='cuda') * (w - time_shape + 1)).int()
...
def stack_subsample_frames_no_sync(x, x_lens, stacking=1, subsampling=1):
    ...
    x = x.transpose(1, 2)
    x_lens = torch.div(x_lens.int() + stacking - 1,
                       stacking, rounding_mode='trunc')
    return x, x_lens
