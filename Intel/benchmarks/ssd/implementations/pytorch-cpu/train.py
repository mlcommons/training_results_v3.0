...
from mlperf_logging.mllog.constants import (SUBMISSION_BENCHMARK, SUBMISSION_ORG, SUBMISSION_PLATFORM, SUBMISSION_DIVISION, SUBMISSION_STATUS,
...
def parse_args(add_help=True):
    ...
    parser.add_argument('--verbose-timing', dest="verbose_timing", action="store_true")
    parser.add_argument('--no-eval', dest="no_eval", action="store_true")
    parser.add_argument('--num-steps', default=0, type=int)
    parser.add_argument('--ipex-optimize', dest="ipex_optimize", action="store_true")
    parser.add_argument('--bf16', dest="bf16", action="store_true")
    parser.add_argument('--distributed', dest="distributed", action="store_true")
    parser.add_argument('--dist-backend', dest="dist_backend", default='ccl')
    parser.add_argument('--fl-optimize', dest="fl_optimize", action="store_true")
    parser.add_argument('--fl-index', dest="fl_index", action="store_true")
...
def main(args):
    ...
    os.environ['VERBOSE_TIMING'] = "True" if args.verbose_timing else "False"
    os.environ['FL_OPTIMIZE'] = "True" if args.fl_optimize else "False"
    os.environ['FL_INDEX'] = "True" if args.fl_index else "False"
    ...
    mllogger.event(key=SUBMISSION_PLATFORM, value="Intel(R) Xeon(R) Platinum 8480+")
    ...
    if args.ipex_optimize is True:
        prec = torch.bfloat16 if args.bf16 else torch.float32
        model, optimizer = ipex.optimize(model, dtype=prec, optimizer=optimizer, weights_prepack=False)

    if args.distributed:
        utils.dist._DEFAULT_FIRST_BUCKET_BYTES = 0 *1024 * 1024
        model = torch.nn.parallel.DistributedDataParallel(model, bucket_cap_mb=10, find_unused_parameters=False, broadcast_buffers=False, gradient_as_bucket_view=True)
    ...
    data_loader = torch.utils.data.DataLoader(
        ...
        pin_memory=False, collate_fn=utils.collate_fn)
