...
def train_one_epoch(model, optimizer, data_loader, device, epoch, args):
    verbose_timing = True if args.verbose_timing else False
    ...
    for images, targets in metric_logger.log_every(data_loader, args.print_freq, header):
        ...
        if verbose_timing:
            timer_1 = time.time()
            #print("Data_Load: " + str(timer_1 - timer_0))

        with torch.cpu.amp.autocast(enabled=args.amp):
        ...
        if verbose_timing:
            timer_2 = time.time()
            #print("FWD+Loss: " + str(timer_2 - timer_1))
        ...
        if verbose_timing:
            timer_3 = time.time()
            #print("BWD: " + str(timer_3 - timer_2))

        optimizer.step()

        if verbose_timing:
            timer_4 = time.time()
            #print("Optimizer: " + str(time.time() - timer_3))
            print(f"Dataload: {str(timer_1 - timer_0)} FWD+Loss: {str(timer_2 - timer_1)} BWD: {str(timer_3 - timer_2)} Opt: {str(timer_4 - timer_3)}")

        if lr_scheduler is not None:
            lr_scheduler.step()

        if (args.num_steps > 0) and (step_count > 99) and (step_count < 200):
            iters_time += time.time() - st_time
        ...
        if (args.num_steps > 0) and (step_count > args.num_steps):
            print("Avg time for 100 iters = ", (iters_time/100))
            break
        st_time = time.time()
...
def evaluate(model, data_loader, device, epoch, args):
    ...
    print("DEBUG: eval_batch_size: " + str(args.eval_batch_size))
