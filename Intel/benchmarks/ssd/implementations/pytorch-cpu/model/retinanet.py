...
class RetinaNetClassificationHead(nn.Module):
    ...
    def compute_loss(self, targets, head_outputs, matched_idxs):
        ...
        fl_index = True if os.environ.get('FL_INDEX') == "True" else False
        ...
        for targets_per_image, cls_logits_per_image, matched_idxs_per_image in zip(targets, cls_logits, matched_idxs):
            ...
            if fl_index:
                ...
                inverse_valid_idxs_per_image = matched_idxs_per_image == self.BETWEEN_THRESHOLDS
                ...
                input_invalid = cls_logits_per_image[inverse_valid_idxs_per_image]
                target_invalid = gt_classes_target[inverse_valid_idxs_per_image]
                loss2 = sigmoid_focal_loss(
                        input_invalid,
                        target_invalid,
                        reduction='sum',
                )
...
class RetinaNet(nn.Module):
    ...
    def postprocess_detections(self, head_outputs, anchors, image_shapes):
        ...
        for index in range(num_images):
            ...
            image_boxes = torch.cat(image_boxes, dim=0).float()
            image_scores = torch.cat(image_scores, dim=0).float()
    ...
    def validate_inputs(self, images, targets=None):
    ...
    def forward(self, images, targets=None):
        ...
        if self.training:
            ...
            # compute the losses
            verbose_timing = True if os.environ.get('VERBOSE_TIMING') == "True" else False
            if verbose_timing is True:
                loss_time_start = time.time()

            losses = self.compute_loss(targets, head_outputs, anchors)

            if verbose_timing is True:
                print("Loss: " + str(time.time() - loss_time_start))
        else:
            # recover level sizes
