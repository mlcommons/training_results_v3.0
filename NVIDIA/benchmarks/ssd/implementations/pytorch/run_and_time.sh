...
if [ ${NVTX_FLAG} -gt 0 ]; then
  ...
  PRE_CMD=('/nsight/bin/nsys' 'profile' '--capture-range' 'cudaProfilerApi' '--capture-range-end' 'stop' '--sample=none' '--cpuctxsw=none' '--trace=cuda,nvtx' '--stats' 'true' '--cuda-graph-trace=node' '-f' 'true' '-o' ${NSYS_OUT})
