...
DESCRIPTION
        ...
        -ep, --enable-profiling
            if set, this flags enables additional profiling runs.
            default: true
...
# output files for console logging for profile run and train/eval
DESC_FILE=$OUTPUT_DIR/desc.txt
PROFILE_LOG_FILE=$OUTPUT_DIR/profile.log
TRAIN_LOG_FILE=$OUTPUT_DIR/train.log
EVAL_LOG_FILE=$OUTPUT_DIR/eval.log

# output directories for profiling run and train/eval
RESULTS_DIR_FOR_PROFILING=$OUTPUT_DIR/results_prof
RESULTS_DIR_FOR_TRAIN_EVAL=$OUTPUT_DIR/results
RESULTS_DIR_FOR_INIT_CKPT_EVAL=$OUTPUT_DIR/result_init_ckpt

# checkpoint dirs for profiling run and train/eval
CKPT_DIR_FOR_PROFILING=$RESULTS_DIR_FOR_PROFILING/checkpoints
...
# dlloger output files for profiling run and train/eval
DLLOGER_OUT_PROFILING=$RESULTS_DIR_FOR_PROFILING/dlloger.json
DLLOGER_OUT_TRAIN_AND_EVAL=$RESULTS_DIR_FOR_TRAIN_EVAL/dlloger.json

# tensorboard directory for profiling run and train
TB_DIR_PROFILE=$OUTPUT_DIR/tb_profile
...
read -r -d '' PREPARE_CMD << EOM
    if [[ \$OMPI_COMM_WORLD_LOCAL_RANK == 0 ]]; then \
        ...
        rm -rf $OUTPUT_DIR ; \
        mkdir -p $CKPT_DIR_FOR_PROFILING ; \
        mkdir -p $TB_DIR_PROFILE ; \
...
if [ "$ENABLE_PROFILING" == "true" ]; then
    echo
    echo 'Evaluating initial checkpoint'
    echo
    set -x
    $MPIRUN_CMD python3 $SCRIPT_DIR/run_pretraining.py \
        --config_file $SCRIPT_DIR/bert_config.json \
        --output_dir $RESULTS_DIR_FOR_INIT_CKPT_EVAL \
        --do_eval \
        ...
        --distribution_strategy allreduce_post_accumulation  2>&1 | tee $EVAL_LOG_FILE
    set +x

    echo
    echo 'Gathering graph dumps'
    echo
    set -x
    rm -rf .graph_dumps
    GRAPH_VISUALIZATION=1 mpirun -n 1 --bind-to core --map-by socket:PE=$MPI_MAP_BY_PE --rank-by core --report-bindings --allow-run-as-root \
        python3 $SCRIPT_DIR/run_pretraining.py \
        --config_file $SCRIPT_DIR/bert_config.json \
        --output_dir $CKPT_DIR_FOR_PROFILING \
        --do_train \
        --json-summary $DLLOGER_OUT_TRAIN_AND_EVAL \
        --use_fused_lamb \
        --use_habana \
    ...
    tar -cvzf $TB_DIR_PROFILE/graph_dumps.tar.gz .graph_dumps/*
    set +x
    ...
    echo
    set -x
    $MPIRUN_CMD rm -rf $CKPT_DIR_FOR_PROFILING
    $MPIRUN_CMD mkdir -p $CKPT_DIR_FOR_PROFILING
    python3 /software/synrec/synrec.py -p PytorchBertMlperf -- mpirun -n 1 --bind-to core --map-by socket:PE=$MPI_MAP_BY_PE --rank-by core --report-bindings --allow-run-as-root \
        python3 $SCRIPT_DIR/run_pretraining.py \
        --config_file $SCRIPT_DIR/bert_config.json \
        --output_dir $CKPT_DIR_FOR_PROFILING \
        --do_train \
        ...
        $DISTRIBUTION_OPTION \
        --tensorboard_dir $TB_DIR_TRAIN  2>&1 | tee $TRAIN_LOG_FILE
    tar -cvzf $TB_DIR_PROFILE/pt_bert_mlperf_synrec.tar.gz PytorchBertMlperf*
    set +x

    echo
    echo 'Running first steps of the training with the profiling enabled'
    echo
    set -x
    $MPIRUN_CMD rm -rf $CKPT_DIR_FOR_PROFILING
    $MPIRUN_CMD mkdir -p $CKPT_DIR_FOR_PROFILING
    $MPIRUN_CMD rm -rf /tmp/profile

    HABANA_PROFILE=profile_api HABANA_PROF_CONFIG=$SCRIPT_DIR/prof_config.json $MPIRUN_CMD python3 $SCRIPT_DIR/run_pretraining.py \
        --config_file $SCRIPT_DIR/bert_config.json \
        --output_dir $CKPT_DIR_FOR_PROFILING \
        --do_train \
        --json-summary $DLLOGER_OUT_PROFILING \
        --use_fused_lamb \
        --use_habana \
        --hmp \
        --hmp_bf16 $SCRIPT_DIR/ops_bf16_bert_pt.txt \
        ...
        --checkpoint_filter model \
        $DISTRIBUTION_OPTION \
        --tensorboard_dir $TB_DIR_PROFILE \
        --profile_steps 20,21  2>&1 | tee $PROFILE_LOG_FILE
    tar -cvzf $TB_DIR_PROFILE/pt_bert_mlperf_hw_profile.tar.gz /tmp/profile/*
    set +x

    echo
    echo 'Running first steps of the training with the tensorboard profiling enabled'
    echo
    set -x
    $MPIRUN_CMD rm -rf $CKPT_DIR_FOR_PROFILING
    $MPIRUN_CMD mkdir -p $CKPT_DIR_FOR_PROFILING

    HABANA_PROFILE=profile_api $MPIRUN_CMD python3 $SCRIPT_DIR/run_pretraining.py \
        --config_file $SCRIPT_DIR/bert_config.json \
        --output_dir $CKPT_DIR_FOR_PROFILING \
        --do_train \
        --json-summary $DLLOGER_OUT_PROFILING \
