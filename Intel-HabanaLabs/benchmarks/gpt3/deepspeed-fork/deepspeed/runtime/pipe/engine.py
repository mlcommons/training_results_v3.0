...
from ..utils import PartitionedTensor, get_use_hpu
...
class PipelineEngine(DeepSpeedEngine):
    ...
    def __init__(self, has_bool_tensors=False, *super_args, **super_kwargs):
        ...
        assert (unique_params_quotient * chunk_size + unique_params_remainder) == unique_params, "Value mismatch after Int64 splitting"
        assert (num_params_quotient * chunk_size + num_params_remainder) == num_params, "Value mismatch after Int64 splitting"

        params_tensor = torch.IntTensor(data=[num_params_quotient, num_params_remainder,
                                              unique_params_quotient, unique_params_remainder]).to(self.device)
    ...
    def eval_batch(self,
        ...
        if self.is_last_stage():
            ...
            self.print_pipe(f"eval_batch after _reduce_outputs | {eval_output=}")
        htcore.mark_step()
        ...
        if self.global_rank == 0 and self.monitor.enabled:
            ...
            self.monitor.write_events(self.summary_events)
        htcore.mark_step()
        # RestorSe the training iterator
    ...
    def print_pipe(self, string):
        print(f"[PIPE_DEBUG][rank={self.global_rank}] {string}")
        ...
        if reduce.lower() == 'avg':
            ...
            if torch.is_tensor(outputs[0]):
                ...
                for output in outputs:
                    item_outputs.append(output.item())
                reduced = sum(outputs)
                reduced_item = sum(item_outputs)
            else:
            ...
            self.print_pipe(f"_reduce_outputs before _scale_loss_by_gas | continue using {reduced_item=} | overriding {reduced=}, ")
            reduced = torch.tensor(reduced_item, dtype=reduced.dtype, device=reduced.device)
            reduced = self._scale_loss_by_gas(reduced, eval_micro_batches)
            self.print_pipe(f"_reduce_outputs after _scale_loss_by_gas | {reduced=}")
            dist.barrier(group=self.mpu.get_data_parallel_group())
            if reduced.item() > 5 or reduced.item() < 1:
                self.print_pipe(f"Printing outputs due to bad loss, {outputs=}")
            dist.barrier(group=self.mpu.get_data_parallel_group())
            ...
            if reduce_dp and self.is_data_parallel:
                if torch.is_tensor(reduced):
                    ...
                    self.print_pipe(f"_reduce_outputs after all_reduce | {reduced=}")
                    reduced /= self.dp_world_size
                    self.print_pipe(f"_reduce_outputs after div dp_world_size | {reduced=}")
                else:
                    for idx in range(len(reduced)):
                        dist.all_reduce(reduced[idx],
                                        group=self.mpu.get_data_parallel_group())
                        self.print_pipe(f"_reduce_outputs after all_reduce {idx=} | {reduced[idx]=}")
                        reduced[idx] /= self.dp_world_size
                        self.print_pipe(f"_reduce_outputs after div dp_world_size {idx=} | {reduced[idx]=}")
            htcore.mark_step()
            return reduced
        else:
