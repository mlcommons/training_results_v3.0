...
from deepspeed.runtime.utils import copy_to_device, move_to_device, see_memory_usage, bwc_tensor_model_parallel_rank, get_use_hpu
...
def model_parallel_cuda_manual_seed(seed):
    ...
    get_cuda_rng_tracker().manual_seed(data_parallel_seed)
...
class CheckpointFunction(torch.autograd.Function):
    ...
    def forward(ctx, run_function, all_outputs, *args):
        ...
        if SYNCHRONIZE:
            if args[0].device.type == 'hpu':
                ...
                # TODO SW-93440: check if its nessicary to call synchronize
        ...
        if device is None:
            ...
            if device.type == 'cuda':
                ...
                import habana_frameworks.torch as htorch
                transport_stream = htorch.hpu.Stream(device=device)
        ...
        # Copy the rng states.
        ctx.fwd_rng_state = get_device_rng_state()
    ...
    def backward(ctx, *grads):
        ...
        bwd_rng_state = get_device_rng_state()

        # Set the states to what it used to be before the forward pass.
        set_device_rng_state(ctx.fwd_rng_state, device)
        ...
        set_device_rng_state(bwd_rng_state, device)
