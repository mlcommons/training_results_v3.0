...
from torch.distributed.distributed_c10d import (
    GroupMember, BroadcastOptions, AllreduceOptions, ReduceOp
)
...
class SimProcessGroup:
    ...
    def broadcast(self, tensors, opts):
        ...
        if self.backend == 'nccl':
            ...
            tensors = self._to_device(tensors, device)
        elif self.backend == 'hccl':
            tensors = self._to_device(tensors, torch.device('hpu'))
...
def install_sim_dist_backend(sim_world_size, sim_rank):
    ...
    def wrapped_dist_init_process_group(
        ...
        orig_dist_init_process_group(
            backend=SimProcessGroup.BACKEND,
            timeout=timeout,
            world_size=sim_world_size,
            rank=sim_rank,
            store=store,
    ...
    def wrapped_dist_new_group(ranks=None,
                               timeout=default_pg_timeout,
                               backend=None,
                               pg_options=None):
        SimProcessGroup.store_add_rest_of_world(next_group=True)
        pg = orig_dist_new_group(ranks=ranks,
                                 timeout=timeout,
                                 backend=backend,
                                 pg_options=pg_options)
