...
def invalid_model_task_config(model_w_task, dtype, enable_cuda_graph):
    ...
    elif (not bool(pytest.use_hpu) == True) and enable_cuda_graph and pkg_version.parse(
...
class TestModelTask(DistributedTest):
    ...
    def test(
        ...
        if dtype == torch.bfloat16:
            pipe = pipeline(task, model=model, device=-1,
                            framework="pt", torch_dtype=torch.bfloat16)
        else:
        ...
        if bool(pytest.use_hpu) == True:
            pipe.model = deepspeed.init_inference(
                ...
                injection_policy={
                    "BertLayer": (module_inject.HFBertLayerPolicy,)},
            )
        else:
            pipe.model = deepspeed.init_inference(
                 pipe.model,
...
class TestLMCorrectness(DistributedTest):
    ...
    def test(self, model_family, model_name, task):
        ...
        if bool(pytest.use_hpu) == True:
            ...
            dev_name = f"cuda:{local_rank}"
            device = torch.device(f"cuda:{local_rank}")
        dtype = torch.float
        task_dict = lm_eval.tasks.get_task_dict([task])
        if 'gpt-j-6B' in model_name and not bool(pytest.use_hpu) == True:
            dtype = torch.half
