        ...
        if args.log_optimizer_states_to_tensorboard and optimizer is not None:
            ...
            if args.tensor_model_parallel_size > 1:
                ...
                torch.distributed.all_reduce(
                    opt_stats, group=mpu.get_tensor_model_parallel_group(), async_op=args.use_hpu)
                opt_stats_2 = torch.FloatTensor(opt_stats_2).to(get_current_device(),)
                torch.distributed.all_reduce(opt_stats_2, op=torch.distributed.ReduceOp.MAX,
            ...
            if args.pipeline_model_parallel_size > 1:
                opt_stats = torch.FloatTensor(opt_stats).to(get_current_device())
                torch.distributed.all_reduce(
                    opt_stats, group=mpu.get_pipeline_model_parallel_group(), async_op=args.use_hpu)
                opt_stats_2 = torch.FloatTensor(opt_stats_2).to(get_current_device())
                torch.distributed.all_reduce(opt_stats_2, op=torch.distributed.ReduceOp.MAX,
...
def evaluate_and_print_results(prefix, forward_step_func,
    ...
    if mpu.is_pipeline_last_stage():
        ...
        torch.distributed.all_reduce(eval_loss_tensor, op=torch.distributed.ReduceOp.SUM, async_op=args.use_hpu, group=mpu.get_data_parallel_group())
        eval_loss_tensor /= mpu.get_data_parallel_world_size()
    else:
        eval_loss_tensor = torch.FloatTensor([0.0]).to(get_current_device())
