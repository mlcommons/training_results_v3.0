...
if [[ -n "${SLURM_LOCALID-}" ]] && [[ "${SLURM_NTASKS}" -gt "${SLURM_JOB_NUM_NODES}" ]]; then
    ...
    if [[ "${DGXSYSTEM}" == DGXA100* ]]; then
        CMD=( 'bindpcie' ${IB_BIND} '--cpu=dgxa100_topology.sh' '--mem=dgxa100_topology.sh' '--' ${NSYSCMD} 'python' '-u' )
    else
...
if [ ${USE_TORCHRUN} -gt 0 ]; then
    if [ ${DGXNGPU} -eq 1 ]; then
	CMD=( ${NSYSCMD} 'python' '-u' )
    else
...
${LOGGER:-} "${CMD[@]}" maskrcnn/tools/train_mlperf.py \
  ${EXTRA_PARAMS} \
  --config-file 'maskrcnn/configs/e2e_mask_rcnn_R_50_FPN_1x_pyt.yaml' \
  DTYPE 'float16' \
