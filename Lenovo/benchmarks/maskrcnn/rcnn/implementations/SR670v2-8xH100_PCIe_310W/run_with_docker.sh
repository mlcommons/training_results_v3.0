...
_cont_mounts=("--volume=${DATADIR}:/data" "--volume=${LOGDIR}:/results" "--volume=${PKLPATH}:/pkl_coco")
...
mllogger.mlperf_submission_log(mllogger.constants.MASKRCNN)"
	    ...
	    docker exec -it "${_cont_name}" ln -sTf /data/coco2017 /coco
	    docker exec -it "${_cont_name}" python maskrcnn/dataset_scripts/create_coco2017_pyt_dataset.py
            docker exec -it "${_cont_name}" python -c "
from maskrcnn_benchmark.utils.mlperf_logger import mllogger
