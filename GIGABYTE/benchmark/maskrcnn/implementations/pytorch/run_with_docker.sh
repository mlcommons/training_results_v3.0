...
_cont_mounts=("--volume=${DATADIR}:/data" "--volume=${LOGDIR}:/results" "--volume=${PKLPATH}:/pkl_coco" "--volume=$(pwd):/workspace/object_detection" "--volume=/dev/shm/coco/coco_train2017_pyt:/coco_train2017_pyt")
...
docker run --rm --gpus all --init --detach \
    ...
    --ulimit=stack=67108864 --ulimit=memlock=-1 -e MLPERF_SUBMISSION_ORG=GIGABYTE -e MLPERF_SUBMISSION_PLATFORM=G593-SD0 \
