...
def timed(verbose=False):
    def timed_dec(fn):
        ...
        def wrapped(*args, **kwargs):
            ...
            name = fn.__name__
            if verbose:
                print(f'[{rank}]: {name} init')
            start = time.time()
            ret = fn(*args, **kwargs)
            took = time.time() - start
            if verbose:
                print(f'[{rank}]: {name} took {took}s')
...
class TwoStageDataParallelLoadShardedStrategy(LoadShardedStrategy):
    ...
    def summarize_load_times(self):
        # print(torch.distributed.get_rank(), 'Done')
        ...
        for key, times in sorted(timers.items()):
            ...
            if torch.distributed.get_rank() == 0:
                print(f'[load times] {key}: max {max_times[0]}, avg {avg_times[0]}')
    ...
    def maybe_init_gloo_group(self):
        ...
        for group_ranks in sorted(all_groups):
            # print(f'New group {torch.distributed.get_rank()}:, {group_ranks}')
    ...
    def _exchange_loaded_tensors(self, extended_tensors: List[_ExtendedShardedTensor], sharded_state_dict, checkpoint_dir) -> List[_ExtendedShardedTensor]:
        # print(f'_exchange_loaded_tensors, num ext_tensors: {len(extended_tensors)}')
