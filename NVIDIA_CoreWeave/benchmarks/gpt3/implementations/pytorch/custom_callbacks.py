...
from nemo.collections.nlp.data.language_modeling.megatron.gpt_dataset import \
    _create_ltor_masks_and_position_ids, build_train_valid_test_datasets
...
def run_training_warmup(trainer, warmup_train_steps):
    ...
    torch.distributed.barrier()
    logging.info(f'[{torch.distributed.get_rank()}]: Time spent in run_training_warmup: {time.time() - start}s')
...
class CustomCallback(Callback):
    def __init__(self, force_success_status):
        super().__init__()
        if force_success_status:
    ...
    def on_train_epoch_start(self, trainer: "pl.Trainer", pl_module: "pl.LightningModule") -> None:
        ...
        mllogger.start(key=mllogger.constants.BLOCK_START,
                       metadata={'first_epoch_num': trainer.current_epoch,
                       'epoch_count': trainer.val_check_interval * pl_module.cfg.global_batch_size * pl_module.cfg.data.seq_length},
                       sync=False)
    ...
    # # TODO: decide if we want to log RUN_START before or after datasets building. If "before", then uncomment this method and remove "on_train_start"
    # def setup(self, trainer: "pl.Trainer", pl_module: "pl.LightningModule", stage: str) -> None:
    #     # This method is called just before building the datasets
    #     # Note: run on all ranks (to allow synchronization)
    #     if stage == 'fit':
...
class MetricsLogger(Logger):
    ...
    def _log_val_metrics(self, metrics: Dict[str, float],
        ...
        if val_loss <= self.target_val_log_ppl:
            logging.info(f'Target Log PPL {self.target_val_log_ppl} reached, stopping training')
            # TODO: sync workers?
...
class DistributedCheckpointIO(TorchCheckpointIO):
    ...
    def save_dist_checkpoint(self, checkpoint: Dict[str, Any], path: _PATH,
        ...
        torch.distributed.barrier()
        logging.info(f'[{torch.distributed.get_rank()}]: Time spent in init_save_checkpoint: {time.time() - start}s')
        dist_checkpointing.save(checkpoint, path)
        torch.distributed.barrier()
        logging.info(f'[{torch.distributed.get_rank()}]: Time spent in save_checkpoint: {time.time() - start}s')
    ...
    def load_dist_checkpoint(self, path: _PATH, lightning_module: pl.LightningModule) -> Dict[str, Any]:
        ...
        torch.distributed.barrier()
        logging.info(f'[{torch.distributed.get_rank()}]: Time spent in load_checkpoint: {time.time() - start}s')
        return state_dict
    ...
    def remove_dist_checkpoint(self, path: _PATH) -> None:
        ...
        logging.info(f'[{torch.distributed.get_rank()}]: Time spent in remove_checkpoint: {time.time() - start}s')
...
class CustomNLPDDPStrategy(NLPDDPStrategy):
    ...
    def load_checkpoint(self, checkpoint_path: _PATH) -> Dict[str, Any]:
        ...
        if 'pytorch-lightning_version' in ckpt:
            logging.info(f'Detected resuming from a PTL checkpoint step {ckpt["global_step"]}.'
                         f' Resetting loop counters.')
            ckpt = self.reset_steps_and_loops(ckpt)
        else:
            logging.info('Detected resuming from reference checkpoint.')
    ...
    def inject_gbs_dependent_values(self, ckpt):
        ...
        logging.info(f'Injecting the following hparams: {step=}, {last_lr=}, {initial_lr=}')
...
class CustomMegatronGPTModel(MegatronGPTModel):
    ...
    def setup_data_mmap(self):
        if self.cfg.data.get('delay_data_mmap', False):
            if self._train_ds:
    ...
    def _register_sharded_tensor_state_dict_hooks_if_available(self) -> None:
        logging.info('Overriding _register_sharded_tensor_state_dict_hooks_if_available'
    ...
    def _extract_consumed_samples_from_ckpt(self, ckpt_path):
        ...
        if consumed_samples == 0 and self.cfg.custom.override_zero_consumed_samples:
            ...
            logging.info(f'Overriding consumed_samples from 0 to {consumed_samples}')
        return consumed_samples

    def build_train_valid_test_datasets(self):
        """ Original method, only replacing `self.trainer.max_steps` with `self.cfg.custom.dataset_max_steps`. """
        # TODO: this is a temporary change for debugging purposes, if we want to use it it should be implemented in NeMo
        if self.cfg.custom.dataset_max_steps == 0:
            return super().build_train_valid_test_datasets()

        if self.cfg.custom.override_zero_consumed_samples:
            raise ValueError('Non-zero `self.cfg.custom.dataset_max_steps` requires `self.cfg.custom.override_zero_consumed_samples=False`')

        logging.warning('Running custom build_train_valid_test_datasets method.')

        logging.info('Building GPT datasets.')
        if self.trainer.limit_val_batches > 1.0 and isinstance(self.trainer.limit_val_batches, float):
            raise ValueError("limit_val_batches must be an integer or float less than or equal to 1.0.")
        global_batch_size = self.cfg.global_batch_size
        max_train_steps = self.cfg.custom.dataset_max_steps  # NOTE: this is the only change
